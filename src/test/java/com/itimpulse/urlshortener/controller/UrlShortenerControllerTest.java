...
@WebMvcTest(controllers = UrlShortenerController.class, excludeAutoConfiguration = {JpaAuditingConfig.class})
class UrlShortenerControllerTest {
    ...
    void testWhenCustomIdAlreadyExists() throws Exception {
        ...
        when(urlShortenerService.createShortUrl(any(), any()))
                .thenThrow(new ConflictException("The provided ID already exists. Please choose a different ID."));
    ...
    void testWhenShortUrlIsExpired() throws Exception {
        ...
        when(urlShortenerService.getShortUrl(expiredId))
                .thenThrow(new UrlExpiredException("The requested short URL has expired and is no longer accessible."));
    ...
    void testDeleteShortUrl() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/shorten-url/" + id))
                .andExpect(status().isOk())
